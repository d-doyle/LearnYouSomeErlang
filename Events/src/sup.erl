...
%%% Called with SupPid = sup:start(evserv, []).
start(Mod, Args) ->
	spawn(?MODULE, init, [{Mod, Args}]).

%%% Spawn a new linked supervisor process for this module with arguments
%%% Called with SupPid = sup:start_link(evserv, []).
start_link(Mod, Args) ->
...
init({Mod, Args}) ->
	...
	%% Call loop function to have it start a new process for Mod (evserv) 
	%% and start_link with Args ([])
	loop({Mod, start_link, Args}).

loop({M, F, A}) ->
	%% Call the Module (evserv), function (start_link), passing in the Arguments ([])
	Pid = apply(M, F, A),
